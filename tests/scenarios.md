...
## 20. Narrow skill selection

Prompt set: an isolated Compose alignment bug; an Android state/persistence feature; a read-only Play policy audit; a localized listing rewrite; and a full build-and-publish request.

Expected: select `$android-ui-layout-engineer`, `$android-app-developer`, `$google-play-policy-reviewer`, `$google-play-aso-expert`, and `$android-product-pipeline` respectively. Do not load the end-to-end orchestrator, competitor research, release procedures, or unrelated specialists for an isolated task. The policy reviewer remains read-only, while the release manager is the sole owner of external release mutations. Load only references whose routing condition is present. For the end-to-end request, preserve the multi-stage gates and evidence-based terminal outcome instead of stopping after the first implementation.
