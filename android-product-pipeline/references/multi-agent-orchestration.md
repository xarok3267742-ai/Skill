...
Assign canonical content ownership before implementation: the application writer owns in-app string resources; the ASO writer owns assigned Play listing metadata, listing localizations, and creative-source paths; the policy writer/reviewer owns assigned canonical files under `repo/play/privacy-policy/`. The coordinator integrates those sources before freezing release evidence. Only the release manager publishes them remotely.
